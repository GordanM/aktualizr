...
OstreePackage::OstreePackage(const std::string &ecu_serial_in, const std::string &ref_name_in,
  ...
  if (branch_name.empty() || refhash.empty())
    throw std::runtime_error("malformed OSTree target name: " + ref_name);
}
...
OstreePackage OstreePackage::fromJson(const Json::Value &json) {
  return OstreePackage(json["ecu_serial"].asString(), json["ref_name"].asString(),
