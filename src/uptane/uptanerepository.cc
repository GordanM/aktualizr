...
Repository::Repository(const Config &config_in, INvStorage &storage_in)
  ...
  // ECU serials should be saved during initialization process, if they are missing it is an exceptional case
  if(!storage.loadEcuSerials(&ecu_serials))
    throw std::runtime_error("ECU serials are missing");

  primary_ecu_serial = ecu_serials[0].second;
  // Primary ECU keys should be saved during initialization process, if they are missing it is an exceptional case
  if(!storage.loadPrimaryKeys(&primary_public_key, &primary_public_key))
    throw std::runtime_error("Primary ECU keys are missing");
...
Json::Value Repository::getVersionManifest(Json::Value custom) {
  ...
  Json::Value ecu_version_signed =
      Crypto::signTuf(primary_private_key, primary_public_key, unsigned_ecu_version);
