...
Uptane::Target SotaUptaneClient::findOstree(const std::vector<Uptane::Target> &targets) {
  for (std::vector<Uptane::Target>::iterator it = targets.begin(); it != targets.end(); ++it) {
    // treat empty format as OSTree for backwards compatibility
    if(it->format().empty || it->format() == "OSTREE")
      return *it;
  }
}
    ...
    try {
	  ...
	  *events_channel << boost::make_shared<event::UptaneTargetsUpdated>(updates);
          ...
          *events_channel << boost::make_shared<event::UptaneTimestampUpdated>();
          ...
          std::vector<Uptane::Target> primary_updates = std::find_if(updates.begin, updates.end(), Uptane::Target::forEcu(config.uptane.primary_ecu_serial));
	  if(primary_update.size()) {
		  // assuming one OSTree OS per primary => there can be only one OSTree update
		  std::vector<OstreePackage> main_ostree = findOstree(primary_updates);
		  if(main_ostree.size())
			  OstreeInstall(main_ostree[0]);
